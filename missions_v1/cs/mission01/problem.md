...
3×3 크기의 패턴과 필터를 읽어, MAC 연산으로 각 패턴과 가장 유사한 필터를 찾고 성능을 분석하는 프로그램을 구현하세요.

### 제공 데이터

아래 데이터는 problem.md에 포함된 인라인 데이터입니다. `load_data()` 함수에서 직접 사용하세요.

```json
{
    "patterns": {
        "img_01": [[1, 0, 1], [0, 1, 0], [1, 0, 1]],
        "img_02": [[1, 1, 0], [1, 1, 0], [0, 0, 0]],
        "img_03": [[0, 0, 0], [1, 1, 1], [0, 0, 0]],
        "img_04": [[0.5, 0.5, 0], [0.5, 0.5, 0], [0, 0, 0]]
    },
    "filters": {
    ...
    "labels": {
        ...
        "img_03": "line_pattern",
        "IMG_04": "block_pattern"
    }
}
```

- `patterns`: 매칭 대상 이미지 4개(`img_01`~`img_04`), 각 3×3 정수 또는 실수 배열입니다.
- `filters`: 비교 기준 필터 3개(`cross`, `block`, `line`), 각 3×3 정수 배열입니다.
- `labels`: 패턴별 정답 라벨입니다. 키의 대소문자가 불일치하므로 정규화가 필요합니다.
