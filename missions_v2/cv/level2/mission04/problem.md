## 문항: 이미지 객체 검출 및 데이터 증강을 통한 정확도 향상

### 문제

제공되는 이미지에서 박스(Box) 객체를 검출하고, 데이터 증강 기법을 적용한 앙상블 카운팅으로 검출 정확도를 향상시키는 파이프라인을 구현하세요.
...
└── images/                # 640×480 RGB PNG 이미지 (총 15장)
...
- `images/`: easy/medium/hard 각 5장, 총 15장의 640×480 RGB PNG 이미지입니다. 난이도에 따라 박스 배치와 겹침 정도가 달라집니다.
...
- Sobel 커널(3×3)로 수평/수직 엣지를 검출합니다.
